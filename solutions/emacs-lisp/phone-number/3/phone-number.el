    ...
    (if (or
      ...
      (cond
       ((or (string-match "^1" without-country)
        ...
        (signal 'area-code-starting-with-1-error))
       ((or (string-match "^0" without-country)
            (string-match "^(0" without-country))
        (signal 'area-code-starting-with-0-error))
       ((string-match "[a-zA-Z]" without-country)
        (signal 'letters-in-phone-num-error))
       (t (signal 'punctuations-in-phone-num-error))))))
    ...
    (if
      ...
      (cond
       ((or (string-match "^1" trimmed)
        ...
        (signal 'exchange-code-starting-with-1-error))
       ((or (string-match "^0" trimmed)
            (string-match "^(0" trimmed))
        (signal 'exchange-code-starting-with-0-error))
    ...
    (if
      ...
      (cond
       ((string-match "^\\([0-9]\\{5\\}\\)\s*$" trimmed)
        (signal 'country-code-error))
       ((string-match "^\\([0-9]\\{6,\\}\\)\s*$" trimmed)
        (signal 'long-phone-num-error))
       ((string-match "^\\([0-9]\\{0,3\\}\\)\s*$" trimmed)
        (signal 'short-phone-num-error))
       ((string-match "[a-zA-Z]" trimmed)
        (signal 'letters-in-phone-num-error))
       (t (signal 'punctuations-in-phone-num-error))))))
