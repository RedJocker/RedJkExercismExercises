...
let ``Schedule date using only numbers`` () =
    schedule "7/25/2019 13:45:00" |> should equal (DateTime(2019, 7, 25, 13, 45, 0))
...
let ``Has passed with appointment months ago`` () =
    hasPassed (DateTime.Now.AddMonths(-8)) |> should equal true

[<Fact>]
[<UseCulture("en-US")>]
[<Task(2)>]
let ``Has passed with appointment days ago`` () =
    hasPassed (DateTime.Now.AddDays(-23.0)) |> should equal true

[<Fact>]
[<UseCulture("en-US")>]
[<Task(2)>]
let ``Has passed with appointment hours ago`` () =
    hasPassed (DateTime.Now.AddHours(-12.0)) |> should equal true
...
let ``Has passed with appointment in 1 minute`` () =
    hasPassed (DateTime.Now.AddMinutes(1.0)) |> should equal false

[<Fact>]
[<UseCulture("en-US")>]
[<Task(2)>]
let ``Has passed with appointment in minutes`` () =
    hasPassed (DateTime.Now.AddMinutes(5.0)) |> should equal false

[<Fact>]
[<UseCulture("en-US")>]
[<Task(2)>]
let ``Has passed with appointment in days`` () =
    hasPassed (DateTime.Now.AddDays(19.0)) |> should equal false

[<Fact>]
[<UseCulture("en-US")>]
[<Task(2)>]
let ``Has passed with appointment in months`` () =
    hasPassed (DateTime.Now.AddMonths(10)) |> should equal false
